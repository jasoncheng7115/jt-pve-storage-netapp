...
## ONTAP HA Best Practices

For production deployments, configure the ONTAP SVM to survive a controller failure without I/O interruption.
...
```
WARNING: Storage 'netapp1' SVM has only 1 iSCSI LIF -- no path redundancy.
```

### Recommended LIF layout

For a 2-controller HA pair:

| LIF | Home node | Home port |
|-----|-----------|-----------|
| iscsi_lif1 | node-01 | e0a |
| iscsi_lif2 | node-02 | e0a |
| iscsi_lif3 | node-01 | e0b |
| iscsi_lif4 | node-02 | e0b |

This gives 4 paths total. Lose any one component (controller, port, cable, switch) and at least 2 paths remain.

### Verify your configuration

```bash
# On ONTAP CLI
network interface show -vserver <svm> -services data_iscsi
# Expected: 2+ LIFs, distributed across home-node values

# Check LIF current location and failover targets
network interface show -vserver <svm> \
    -fields failover-policy,home-node,current-node
# All LIFs should currently be on their home-node (no recent failover)
```

### Multipath configuration on the host

On each Proxmox VE node, multipath must see all LIFs:

```bash
multipath -ll | grep -A6 NETAPP
```

You should see one entry per LUN, with paths from all LIFs:

```
3600a09807770457a795d2f314b58524a dm-9 NETAPP,LUN C-Mode
...
If fewer paths than LIFs, run iSCSI discovery and login on each LIF:

```bash
for lif in 192.168.1.101 192.168.1.102 192.168.1.103 192.168.1.104; do
    iscsiadm -m discovery -t st -p $lif
    iscsiadm -m node -p $lif --login 2>/dev/null
done
```

### LUN reservation timeout

For HA failover scenarios, shorten the LUN reservation timeout from the ONTAP default (60s) to 30s:
...
```
Storage 'netapp1' reachable again after N consecutive failures (~Ns outage)
```
