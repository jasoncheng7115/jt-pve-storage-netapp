...
正式環境部署時，請設定 ONTAP SVM 使其能在 controller 故障時不中斷 I/O。
...
共 4 條路徑。失去任一元件（controller、port、cable、switch）仍至少有 2 條路徑。
...
# 預期：2+ 個 LIF，分散在不同 home-node

# 檢查 LIF 目前位置與 failover 設定
network interface show -vserver <svm> \
    -fields failover-policy,home-node,current-node
# 所有 LIF 應該都在自己的 home-node（沒有近期的 failover 殘留）
```

### Host 端 multipath 設定

在每個 Proxmox VE 節點上，multipath 必須能看到所有 LIF：

```bash
multipath -ll | grep -A6 NETAPP
```

每個 LUN 應該顯示來自所有 LIF 的路徑：
...
若路徑數少於 LIF 數，對每個 LIF 執行 iSCSI discovery 和 login：
...
針對 HA failover 場景，將 LUN reservation 逾時從 ONTAP 預設（60 秒）縮短為 30 秒：
